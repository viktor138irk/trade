...
async def app_bootstrap(db: Session = Depends(get_db)) -> dict:
    ...
    return {
        ...
        'legacy': legacy_bot.dashboard(db),
    }
...
async def create_demo_trade(market: str, side: str, price: float, amount: float, reason: str = 'manual demo trade', db: Session = Depends(get_db)) -> dict:
    try:
...
async def update_bot_settings(enabled: bool | None = None, trade_mode: str | None = None, trade_style_mode: str | None = None, min_signal_score: float | None = None, max_open_positions: int | None = None, max_quote_per_trade: float | None = None, emergency_stop: bool | None = None, db: Session = Depends(get_db)) -> dict:
...
async def create_manual_signal(title: str, market: str, sentiment: str = 'neutral', score: float = 50.0, source: str = 'manual', url: str = '', db: Session = Depends(get_db)) -> dict:
...
async def legacy_dashboard(db: Session = Depends(get_db)) -> dict:
    return legacy_bot.dashboard(db)


@app.get('/api/v1/legacy/open-trades')
async def legacy_open_trades(db: Session = Depends(get_db)) -> dict:
    return {'items': legacy_bot.open_positions(db)}


@app.get('/api/v1/legacy/closed-trades')
async def legacy_closed_trades(limit: int = 50, db: Session = Depends(get_db)) -> dict:
    return {'items': legacy_bot.closed_positions(db, limit)}


@app.get('/api/v1/legacy/chart-history')
async def legacy_chart_history(limit: int = 100, db: Session = Depends(get_db)) -> dict:
    return {'items': legacy_bot.chart_history(db, limit)}


@app.get('/api/v1/legacy/strategies')
async def legacy_strategies(db: Session = Depends(get_db)) -> dict:
    legacy_bot.ensure_defaults(db)
    return {'items': legacy_bot.dashboard(db)['strategies']}


@app.post('/api/v1/legacy/strategies/{strategy_id}')
async def legacy_update_strategy(strategy_id: int, enabled: bool | None = None, dry_run: bool | None = None, db: Session = Depends(get_db)) -> dict:
    try:
...
async def legacy_coinex(db: Session = Depends(get_db)) -> dict:
    return legacy_bot.dashboard(db)['coinex']


@app.post('/api/v1/legacy/coinex')
async def legacy_update_coinex(enabled: bool | None = None, access_id: str | None = None, secret_key: str | None = None, mode: str | None = None, db: Session = Depends(get_db)) -> dict:
    return legacy_bot.update_coinex(db, enabled, access_id, secret_key, mode)


@app.post('/api/v1/legacy/telegram')
async def legacy_update_telegram(enabled: bool | None = None, chat_id: str | None = None, message_format: str | None = None, db: Session = Depends(get_db)) -> dict:
    return legacy_bot.update_telegram(db, enabled, chat_id, message_format)


@app.get('/api/v1/legacy/updater/status')
async def legacy_updater_status(db: Session = Depends(get_db)) -> dict:
    return legacy_bot.updater_status(db)


@app.post('/api/v1/legacy/terminal')
async def legacy_terminal(command: str) -> dict:
    return legacy_bot.terminal_command(command)


@app.get('/api/v1/android/status')
async def android_status(db: Session = Depends(get_db)) -> dict:
